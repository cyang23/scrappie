...
size_t *short_long_peak_detector(DetectorPtr short_detector,
    ...
    if (NULL == short_detector->signal || NULL == long_detector->signal) {
        return NULL;
    }
...
event_t create_event(size_t start, size_t end, double const *sums,
    ...
    event.pos = -1;
    event.state = -1;
    if (NULL == sums || NULL == sumsqs || start >= nsample || end > nsample
        || start >= end) {
...
event_table create_events(size_t const *peaks, double const *sums,
    ...
    if (NULL == peaks || NULL == sums || NULL == sumsqs) {
        return event_tbl;
    }
